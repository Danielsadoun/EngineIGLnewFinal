...
static void glfw_mouse_press(GLFWwindow* window, int button, int action, int modifier)
{
  ...
  if (action == GLFW_PRESS)
  {
	  ...
	  else 
	  {
		  ...
		  scn->is_picked = true;
		  scn->last_picked = scn->selected_data_index;
...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	...
	if (y < 0) {
		if (!rndr->GetScene()->is_picked) {
			rndr->GetScene()->TranslateInSystem(rndr->GetScene()->MakeTrans(), Eigen::Vector3f(0, 0, -0.5), true);
		}
		else {
			...
			//else {
				rndr->GetScene()->data().TranslateInSystem(rndr->GetScene()->MakeTrans(), Eigen::Vector3f(0, 0, -0.5), true);
			//}
		}
	}
	else {
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		{
		case GLFW_KEY_RIGHT:
			//scn->direction = Eigen::Vector3f(0.01, 0, 0);
			/*if (scn->is_picked) {
		...
		case GLFW_KEY_LEFT:
			...
			break;
		case GLFW_KEY_UP:
			//scn->direction = Eigen::Vector3f(0, 0.01, 0);
			/*if(scn->is_picked){
